...
void mostrarResultados(const vector<Bar>& bares, int tiempoDisponible, int estrategia, int valoracionTotal) {
    ...
    switch (estrategia) {
    case 1: nombreEstrategia = "Ordenar por valoración (de mayor a menor)"; break;
    case 2: nombreEstrategia = "Ordenar por tiempo total (de menor a mayor)"; break;
    case 3: nombreEstrategia = "Ordenar por ratio valoración/tiempo (de mayor a menor)"; break;
    }
    ...
    for (size_t i = 0; i < bares.size(); ++i) {
        ...
        int t = b.tiempoTotal();
        cout << i + 1 << ". " << b.nombre
            << " (Valoración: " << b.valoracion
            << ", Tiempo total: " << t << " min)\n";
        tiempoTotalUsado += t;
    }

    cout << "\nValoración total conseguida: " << valoracionTotal << " puntos\n";
...
int main() {
    ...
    mostrarResultados(res1, tiempoDisponible, 1, val1);

    res2 = ejecutarEstrategia(bares, tiempoDisponible, 2, val2);
    mostrarResultados(res2, tiempoDisponible, 2, val2);

    res3 = ejecutarEstrategia(bares, tiempoDisponible, 3, val3);
    mostrarResultados(res3, tiempoDisponible, 3, val3);

    // Elegir estrategia
    int opcion;
    cout << "\n¿Cuál estrategia deseas utilizar? (1, 2 o 3): ";
    cin >> opcion;
